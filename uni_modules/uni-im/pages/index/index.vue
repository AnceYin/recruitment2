	...
	<view id="page" class="uni-im-index">
		...
		<view id="foot" v-if="isWidescreen">
			<view class="item">
				...
				<text>本项目已开源</text>
				<uni-link class="link" href="https://gitcode.net/dcloud/hello-uni-im/-/tree/v3"
					text="git仓库地址"></uni-link>
			</view>
			<view class="item">
		...
		<view id="left-view" v-if="isWidescreen" @click="chatInfoIsShow = false;">
			...
			<uni-badge @contextmenu.prevent.native="openConversationMenu($event,'unreadMsgCount')" class="chat-icon"
				size="small" :text="unreadMsgCount" absolute="rightTop" type="error">
				<uni-icons @click="showChatView" :color="contactsViewIsShow?'#c5c5c5':'#5fc08e'" size="32"
					type="chatbubble-filled"></uni-icons>
			</uni-badge>
			<uni-badge id="show-contacts-btn" size="small" :text="unreadnotificationCount" absolute="rightTop"
				type="error">
		...
		<view id="center-view">
			...
			<template v-if="isWidescreen">
				...
				<view id="search-bar-box" v-show="!contactsViewIsShow">
					<uni-search-bar v-model="keyword" id="search-bar" radius="5" placeholder="搜索" clearButton="auto"
				...
				<!-- 会话查找结果列表 -->
				<uni-im-filtered-conversation-list v-if="keyword" ref="uni-im-filtered-conversation-list"
					id="conversation-list-box" :keyword="keyword" @to-chat="toChat($event)"
					@to-chat-filtered="toChatFiltered($event)"></uni-im-filtered-conversation-list>
			</template>
			<!-- #endif -->

			<!-- 会话用户列表 -->
			<uni-im-conversation-list v-if="!keyword" ref="uni-im-conversation-list" @clickItem="toChat($event.id)"
		...
		<view id="right-view" v-if="isWidescreen">
			...
			<view id="chat-view-box">
				<template v-if="!contactsViewIsShow && currentConversationId">
					<chat-filtered v-if="filteredConversationId" class="content" ref="chat-filtered"
	...
	export default {
		...
		async onLoad(param) {
			uni.requestSubscribeMessage({
				tmplIds: ["BZD93PWRNa9vxm4u0TRevKjMfdJV-njyZSv0wwWdBIw"], // 改成你的小程序订阅消息模板id
				success: () => {
					uni.showToast({
						title: "订阅成功",
						icon: "none"
			...
			if (tokenExpired < Date.now()) {
				...
				let url = '/uni_modules/uni-id-pages/pages/userinfo/userinfo'
